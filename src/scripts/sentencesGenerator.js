import { UKRAINIAN_MODE, ENGLISH_MODE } from "../constants/Constants";
import {
  ENGLISH_SENTENCES,
  UKRAINIAN_SENTENCES,
...
const sentencesGenerator = (sentencesCount, language) => {
  ...
  if (language === UKRAINIAN_MODE) {
    const UkrainianSentencesList = [];
    for (let i = 0; i < sentencesCount; i++) {
      const rand = randomIntFromRange(0, 55);
      UkrainianSentencesList.push(UKRAINIAN_SENTENCES[rand]);
    }
    return UkrainianSentencesList;
  }
};
